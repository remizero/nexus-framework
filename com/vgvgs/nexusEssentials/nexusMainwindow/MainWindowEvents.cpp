...
void MainWindowEvents::closeEvent ( QCloseEvent *event ) {
  ...
  if ( ( ( MainWindow * ) this->parent )->getSysTrayIcon ()->isVisible () ) {
    ...
    if ( NSLIB_CORE::AppInit::getInstance ()->getAppConfig ()->getSettings ()->value ( "systemtrayicon/systraydefaultmessagetype" ).toString ().compare ( "Information" ) == 0 ) {
      ...
      QMessageBox::information ( this->parent, NSLIB_CORE::AppInit::getInstance ()->getAppConfig ()->getSettings ()->value ( "app/applicationdisplayname" ).toString (),
                                 NSLIB_CORE::AppInit::getInstance ()->getAppConfig ()->getSettings ()->value ( "systemtrayicon/systraymessagebody" ).toString () );

    } else if ( NSLIB_CORE::AppInit::getInstance ()->getAppConfig ()->getSettings ()->value ( "systemtrayicon/systraydefaultmessagetype" ).toString ().compare ( "Warning" ) == 0 ) {

      QMessageBox::warning ( this->parent, NSLIB_CORE::AppInit::getInstance ()->getAppConfig ()->getSettings ()->value ( "app/applicationdisplayname" ).toString (),
                                 NSLIB_CORE::AppInit::getInstance ()->getAppConfig ()->getSettings ()->value ( "systemtrayicon/systraymessagebody" ).toString () );

    } else if ( NSLIB_CORE::AppInit::getInstance ()->getAppConfig ()->getSettings ()->value ( "systemtrayicon/systraydefaultmessagetype" ).toString ().compare ( "Critical" ) == 0 ) {

      QMessageBox::critical ( this->parent, NSLIB_CORE::AppInit::getInstance ()->getAppConfig ()->getSettings ()->value ( "app/applicationdisplayname" ).toString (),
                                 NSLIB_CORE::AppInit::getInstance ()->getAppConfig ()->getSettings ()->value ( "systemtrayicon/systraymessagebody" ).toString () );
    }
