...
XsdDialog::XsdDialog ( QWidget *parent ) {
  ...
  NSLIB_XSD::XsdEngine *xsdEngine = new NSLIB_XSD::XsdEngine ();
  ...
  this->formDialog->setWindowTitle ( ( ( NSLIB_XSD::NameProperty * ) this->xsdElement->getProperty ( "NameProperty" ) )->getValue () );
  ...
  this->domDocument = Utils::Xml::load ( NSLIB_CORE::AppPaths::getInstance ()->getApplicationConfigPath () + "config.xml", true );
...
void XsdDialog::applySlot () {
  ...
  NSLIB_UTILS::Xml::save ( NSLIB_CORE::AppPaths::getInstance ()->getApplicationConfigPath () + "config.xml", this->domDocument );
}
...
NSLIB_XSD::XsdElement *XsdDialog::getXsdElement ( QDomElement element, NSLIB_XSD::XsdElement *xsdElement ) {

  NSLIB_XSD::XsdElement *xsdElementReturn = nullptr;
  if ( ( ( NSLIB_XSD::NameProperty * ) xsdElement->getProperty ( "NameProperty" ) )->getValue ().compare ( element.nodeName () ) == 0 ) {
  ...
  if ( !element.isNull () ) {
    ...
    if ( element.toText ().isNull () ) {
      ...
      if ( !element.parentNode ().toElement ().isNull () && ( element.nodeType () == QDomNode::TextNode ) ) {
        ...
        NSLIB_XSD::XsdElement *xsdElementType = nullptr;
        if ( inputWidget != nullptr ) {

          xsdElementType = this->getXsdElement ( nodeElement, this->xsdElement );
          if ( xsdElementType != nullptr ) {

            NSLIB_XSD::TypeProperty *objTypeProperty = ( NSLIB_XSD::TypeProperty * ) xsdElementType->getProperty ( "TypeProperty" );
            switch ( objTypeProperty->getValue () ) {
              ...
              case NSLIB_XSD::TypeAbs::HEXBINARY: {
                ...
                NSLIB_COLORBOX::ColorBox *lineEditHex = ( NSLIB_COLORBOX::ColorBox * ) inputWidget;
...
QDomDocument XsdDialog::loadXml () {
  ...
  QFile *file = Utils::Files::load ( NSLIB_CORE::AppPaths::getInstance ()->getApplicationConfigPath () + "config.xml" );
  QDomDocument doc;
  if ( file == nullptr ) {
